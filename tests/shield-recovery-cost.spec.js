...
test('Escudo preserva a vida, mas uma falha de raquete encerra o combo', async ({ page }) => {
  ...
  const saved = await page.evaluate(() => window.__GAME_DEBUG__.getState());
  expect(saved.lives).toBe(beforeLives);
  expect(saved.shieldCharges).toBe(0);
  expect(saved.ball.vy).toBeLessThan(0);
  await expect(page.getByRole('status')).toHaveText('Escudo salvou a bola!');
});
...
test('Escudo encerra o streak Elite e exige novo domínio antes de requalificar', async ({ page }) => {
  ...
  await page.evaluate(() => {
    document.getElementById('gameStatus').textContent = 'Escudo salvou a bola!';
  });
  await page.waitForTimeout(0);
